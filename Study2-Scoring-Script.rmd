...
survey_name_filter = "Gender-IER-Ratings*" # we want to filter just our surveys
...
surveys_tidy = surveys_long %>%
  ...
  #filter(nchar(PROLIFIC_PID) > 1) %>% # ADD BACK ONCE ON PROLIFIC
...
extract(survey_name, c("survey_name"), "GenderIER1-(.*)") |> 
  filter(StartDate > "2026-02-06") 
  


ids = surveys_tidy %>%
  #select(PROLIFIC_PID) %>% # add back prolific id when running the REAL pipeline
  select(ResponseId) %>% # add back ResponseId when running the REAL pipeline
  unique() %>%
  mutate(pID = sprintf("IER-Ratings%03d", row_number())) %>%
  #left_join(., unique(select(surveys_tidy, PROLIFIC_PID))) # add back once on prolific
  left_join(., unique(select(surveys_tidy, ResponseId))) # add back once on prolific

surveys_enrolled = surveys_tidy %>% # create dataset of ppts who are enrolled in the survey
  left_join(., ids) %>%
  filter(is.na(failed_attention_1)) # create screener
...
# prep support statements

```{r}
data_ratings_wide <- data_surveys %>%
  pivot_wider(names_from = item, values_from = value)

# Now reshape support ratings into long format
data_ratings_long <- data_ratings_wide %>%
  # Pivot the rating columns (Warm, Better, Understood)
  pivot_longer(
    cols = matches("^\\d+_(Warm|Better|Understood)$"),
    names_to = "rating_key",
    values_to = "rating"
  ) %>%
  mutate(rating = as.numeric(rating)) %>%
  # Extract response number and rating dimension
  extract(rating_key, c("response_num", "rating_dim"), "(\\d+)_(Warm|Better|Understood)") %>%
  # Pivot the response IDs
  pivot_longer(
    cols = matches("^response_id_\\d+$"),
    names_to = "response_id_key",
    values_to = "response_id"
  ) %>%
  # Extract the response ID number
  extract(response_id_key, "response_id_num", "response_id_(\\d+)") %>%
  # Keep only matching response numbers
  filter(response_num == response_id_num) %>%
  # Remove rows with missing ratings or response IDs
  filter(!is.na(rating) & !is.na(response_id)) %>%
  # Select and arrange final columns
  select(pID, set, response_id, response_num, rating_dim, rating) %>%
  arrange(pID, response_num, rating_dim)
data_ratings_long
...
# Need wide format with raters as columns
icc_data <- data_ratings_long %>%
  # Convert rating to numeric
  mutate(rating = as.numeric(rating)) %>%
  # Create a unique identifier for each item being rated
  unite("item_id", response_id, rating_dim, remove = FALSE) %>%
  # Count number of raters per item
  group_by(item_id) %>%
  mutate(n_raters = n()) %>%
  ungroup() %>%
  # Pivot wider so each rater (pID) is a column
  select(item_id, response_id, rating_dim, n_raters, pID, rating) %>%
  pivot_wider(names_from = pID, values_from = rating)

# View the data with rater counts
DT::datatable(icc_data, options = list(pageLength = 10))

# Calculate ICC(2,1) - two-way random effects, single rater
# This is most common for rating studies
icc_results <- icc(
  icc_data %>% select(-item_id, -response_id, -rating_dim, -n_raters),
  model = "twoway",
  type = "agreement",
  unit = "single"
)
...
# Also calculate ICC(2,k) - average of all raters
icc_results_avg <- icc(
  icc_data %>% select(-item_id, -response_id, -rating_dim, -n_raters),
  model = "twoway",
  type = "agreement",
...
print(icc_results_avg)

# Create a summary table
icc_summary <- data.frame(
  Model = c("ICC(2,1) - Single Rater", "ICC(2,k) - Average Raters"),
  ICC = c(icc_results$value, icc_results_avg$value),
  CI_Lower = c(icc_results$lbound, icc_results_avg$lbound),
  CI_Upper = c(icc_results$ubound, icc_results_avg$ubound),
  F_value = c(icc_results$Fvalue, icc_results_avg$Fvalue),
  p_value = c(icc_results$p.value, icc_results_avg$p.value)
)

og_ratings = read_csv("data/responses_with_sets.csv")
...
data_surveys_all |> 
  filter(grepl("Condition|gender", item)) |> 
  select(pID, survey_name, item, value) |> 
  pivot_wider(names_from = item, values_from = value)  |> 
  select(pID, gender, Condition) |> 
...
# Calculate total N
total_n <- n_distinct(plot_data$pID)

# Create plot
plot_data |> 
  ggplot(aes(x = Condition, fill = gender)) +
  geom_bar(position = "dodge") +
  labs(
    x = "Condition", 
    y = "Count", 
    fill = "Gender",
    title = sprintf("Total N = %d", total_n)  # Alternative: add to title
  ) +
  scale_fill_manual(values = palette) +
  plot_aes +
  theme(legend.position = "top")
