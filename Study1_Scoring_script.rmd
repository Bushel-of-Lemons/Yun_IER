...
ids = surveys_tidy %>%
  ...
  left_join(., unique(select(surveys_tidy, PROLIFIC_PID))) 
...
data_text = data_surveys_all %>%
  filter(grepl("Condition|support_response|gender", item)) %>%
  select(pID,survey_name, item, value) %>%
  pivot_wider(names_from = item,
              values_from = value) |>  
  select(pID,gender, survey_name, Condition, starts_with("support_")) %>%
...
data_text_long <- data_text %>%
  ...
  select(pID, gender, Condition, starts_with("support_response"))
...
write_csv(data_text_long, file.path(data_dir, "IER_support_responses.csv"))
```
...
write.csv(scored,file.path(data_dir, "/IER_scored_data.csv"), row.names = FALSE) # scored data
write.csv(data_demo,file.path(data_dir, "/IER_demos_raw.csv"), row.names = FALSE) # raw demo data
write.csv(basic_demos,file.path(data_dir, "/IER_basic_demos_scored.csv"), row.names = FALSE) # basic scored demos (age,race,gender) to be reattached when we analyze the data
```
